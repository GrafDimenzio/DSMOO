...
public struct ChangeStagePacket : IPacket
{
    ...
    public void Serialize(Span<byte> data)
    {
        ...
        MemoryMarshal.Write(
            data[(Constants.WarpIdSize + Constants.StageSize)..(Constants.WarpIdSize + Constants.StageSize + 1)],
            Scenario);
        MemoryMarshal.Write(
            data[(Constants.WarpIdSize + Constants.StageSize + 1)..(Constants.WarpIdSize + Constants.StageSize + 2)],
            SubScenarioType);
    }
    ...
    public void Deserialize(ReadOnlySpan<byte> data)
    {
        ...
        Id = Encoding.UTF8.GetString(data[Constants.StageSize..(Constants.WarpIdSize + Constants.StageSize)])
            .TrimNullTerm();
        Scenario = MemoryMarshal.Read<sbyte>(
            data[(Constants.WarpIdSize + Constants.StageSize)..(Constants.WarpIdSize + Constants.StageSize + 1)]);
        SubScenarioType = MemoryMarshal.Read<byte>(data[
            (Constants.WarpIdSize + Constants.StageSize + 1)..(Constants.WarpIdSize + Constants.StageSize + 2)]);
    }
}