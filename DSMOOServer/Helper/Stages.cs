...
{
    public static readonly Dictionary<string, string> Alias2Stage = new()
    {
    ...
    {
        // alias value
        if (Alias2Stage.TryGetValue(input.ToLower(), out var mapName)) return mapName;
        // exact stage value
        if (IsStage(input)) return input;
    ...
    {
        var result = "";
        foreach (DictionaryEntry item in Alias2Kingdom) result += item.Key + "  ->  " + item.Value + "\n";
