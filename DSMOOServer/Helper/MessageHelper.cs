...
public class MessageHelper
{
    public static CommandResult FormatMessage(PlayerManager.PlayerSearch players, string message)
    {
        ...
        sb.Append(players.Players.Count > 0 ? $"{message}: {string.Join(", ", players.Players.Select(x => x.Name))}\n" : "");
        sb.Append(players.FailedToFind.Count > 0 ? $"Failed to find matches for: {string.Join(", ", players.FailedToFind)}\n" : "");
