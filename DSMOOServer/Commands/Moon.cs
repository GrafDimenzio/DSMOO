...
[Command(
    ...
    Parameters = ["[list/clear/sync/send/collect/set/include/exclude]"]
)]
public class Moon(MoonManager manager, PlayerManager playerManager) : Command
{
    public override CommandResult Execute(string command, string[] args)
    {
        ...
        switch (args[0].ToLower())
        {
            ...
            case "set":
                if (args.Length == 1 || !bool.TryParse(args[1], out var enable))
                    return new CommandResult
                    {
                        ResultType = ResultType.InvalidParameter,
                        Message = "Usage: moon set [true/false]"
                    };

                manager.Enable = enable;
                return "Set moon sync to " + enable;
